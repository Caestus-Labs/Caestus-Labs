...
shadowPlane.position.y = -0.25; // Adjusted for model
...
const bloom = new UnrealBloomPass(new THREE.Vector2(1,1), 0.28, 0.45, 0.88);
...
// Create three lines extending from VR headset to problem cards
...
// Create lines for each problem
const problemLinesMeshes = lineProblems.map((problem, index) => {
  const points = [];
  const divisions = 50;
  ...
  return { line, geometry, material, problem, glowLine, glowMaterial };
});

// Add glowing spheres at the end of each line
const problemSpheres = lineProblems.map((problem) => {
  ...
  return { sphere, material: sphereMaterial, glowMaterial };
});
...
function render(){
  ...
  // Smooth cursor
  mouse.x += (mouse.tx - mouse.x) * CURSOR_DAMPING;
  mouse.y += (mouse.ty - mouse.y) * CURSOR_DAMPING;
